...
for(topic in fea_df$fea){
  ...
  plot_df$fb_max = max(plot_df$fb_score, na.rm=T)
  plot_df$x_fb_max = plot_df$t[which(plot_df$fb_score==max(plot_df$fb_score))][1]
  pl[[topic]] <- ggplot(plot_df) +
    geom_point(aes(x = x_f1_max, y = f1_max), color='black') +
    geom_line(aes(x = t, y = f1_score),color='red') +
    geom_point(aes(x = x_fb_max, y = fb_max), color='black') +
    geom_line(aes(x = t, y = fb_score),color='blue') +
...
threshold_df <- t_df %>% group_by(topic) %>% 
  summarise(threshold = t[which(fb_score==max(fb_score))][1])
...
# ----- evaluation ---- 
bal <- F
eval_pca <- eval_llm(score_df_pca, label_df_gpt4o, bal)
eval_pca$llm <- "pca1_score"
# eval_agree_pca <- eval_llm(score_df_pca, label_df_gpt4o, bal, findoptimal = T)
...
eval_agree_u <- eval_llm(label_df_agreed_union, label_df_gpt4o, bal)
eval_agree_u$llm <- "union_label"
eval_agree_i <- eval_llm(label_df_agreed_inter, label_df_gpt4o, bal)
eval_agree_i$llm <- "inter_label"
eval_agree_adj <- eval_llm(adjust_label_df, label_df_gpt4o, bal)
eval_agree_adj$llm <- "adjust_label"
...
eval_df$llm <- factor(eval_df$llm, 
                      levels = c("pca1_score", "pca_label", 
                                 "adjust_label", "union_label", "inter_label", 
